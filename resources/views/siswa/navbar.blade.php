    ...
    <div class="relative">
        <button @click="open = !open" @click.away="open = false" class="flex items-center gap-3 focus:outline-none hover:bg-white/10 p-1.5 pr-2 rounded-md transition-colors">
            ...
            <span class="text-sm font-medium text-white hidden md:block">
                {{ Auth::guard('siswa')->check() ? Auth::guard('siswa')->user()->nama_lengkap : 'Nama Siswa' }}
            </span>
        ...
        <div x-cloak x-show="open" 
            ...
            <div class="px-4 py-3 border-b border-gray-50 mb-1">
                <p class="text-sm font-medium text-primer">{{ Auth::guard('siswa')->check() ? Auth::guard('siswa')->user()->nama_lengkap : 'Nama Siswa' }}</p>
                <p class="text-xs text-gray-500 truncate">{{ Auth::guard('siswa')->check() ? Auth::guard('siswa')->user()->email : 'siswa@lpk-saitama.com' }}</p>
            </div>
            
            <a href="{{ route('siswa.profil') }}" class="flex items-center px-4 py-2 text-sm text-gray-700 hover:bg-primer/5 hover:text-primer transition-colors">
