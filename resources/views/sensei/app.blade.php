...
<head>
    ...
    <title>@yield('title', 'Portal Sensei LPK')</title>
...
<body class="bg-[#F8FAFC] text-gray-800 font-sans antialiased">
    ...
    @include('sensei.navbar')

    <div class="pt-16 pb-20 md:pb-0 flex">
        @include('sensei.sidebar')
        ...
        <main class="flex-1 md:ml-72 p-4 md:p-8 min-h-[calc(100vh-4rem)] w-full max-w-full overflow-hidden transition-all duration-300">
            @if(isset($dashboard_banners) && $dashboard_banners->count() > 0)
                <div x-data="{ activeSlide: 1, slides: {{ $dashboard_banners->count() }}, init() { if(this.slides > 1) setInterval(() => { this.activeSlide = this.activeSlide === this.slides ? 1 : this.activeSlide + 1; }, 7000); } }" class="mb-6">
                    @foreach($dashboard_banners as $index => $banner)
                        <div x-show="activeSlide === {{ $index + 1 }}" x-transition.opacity.duration.500ms @if($index > 0) style="display: none;" @endif class="bg-sekunder rounded-md p-4 text-white shadow-sm flex items-start gap-4">
